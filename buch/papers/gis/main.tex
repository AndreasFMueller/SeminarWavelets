...
Dafür ist es wichtig, dass wir den Messpfad von der Fehlerursache bis zum Oszilloskop kennen. 
...
Bei der Fehlstelle am Punkt 0 in Abbildung \ref{fig:Teilentaldungspuls} wird wegen der schnellen Entladung des Strompulses eine elektromagnetische Welle abgestrahlt. 
...
\end{equation}
quadriert.
Mit dem Quadrieren wird ein grösserer Signal-Rausch Abstand (SNR) erzielt und das negative Werte mit einbezogen werden.
...
\rhead{Abschnitt}
In diesem Abschnitt wird behandelt, wie die Auswertung der vier Fehlsignalen von den zwei verschiedenen Fehlerarten angegangen wurde. Auf die Resultate und Erkenntnisse aus diesen wird ebenfalls eingegangen. 
...
Hier wird klar dass das Finden von fehlerspezifische Charakteristiken im CWT Plot schwierig sein könnte.
...
Die Signale werden von der Fehlerquelle bis zur Koppelantenne aufgrund von Refelxions- und Dispersionsphänomenen (frequenzabhängige Laufzeiten) extrem verzerrt und die ursprüngliche Form geht verloren.
Die Verzerrung des Signals ist ersichtlich wenn die Bilder vom Signalpuls an der Fehlstelle \ref{fig:Teilentaldungspuls} und dem Signal der Koppelantenne \ref{fig:Zeitsig} verglichen werden.
