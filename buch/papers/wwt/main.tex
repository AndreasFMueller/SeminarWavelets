...
Die Darstellung der gewonnenen Daten konnten einfach mittels Python realisiert werden.
Anbei in Abbildung \ref{fig:rawdata} \space die Plot der Rohdaten aus dem Jahre 2018, wobei nur jeder zehnte Messpunkt verwendet wurde.
...
Die Berechnung der \textit{cwt} wurde mit der numerischen Berechnungs-SoftwaFre Matlab durchgeführt.
...
Bei den Rückgabewerte werden die Wavelet-Werte in \texttt{'wt'} als komplexe Matrix und die approximierten Frequenzen in \texttt{'F'} abgespeichert.
...
Bei den Rohdaten der Frontdurchgängen erkennt man gemeinsame Wechsel im Wind, der Temperatur sowie dem Luftdruck, daher kann in der Wavelet-Transformation ein erkennbare Antwort erwarten werden. 
Diese korrelierenden auftretenden Frequenzen sollten in der \textit{cwt} sichtbar sein.

\subsection{Parallelen zur Kovarianz}

sUm das korrelierende Auftreten der einzelnen Datenkanäle zu verdeutlichen, wurden bei diesen jeweils die zwei zusammengehörenden Messresultate miteinander multipliziert. 
...
\begin{figure}
	\centering
	\lstinputlisting[language=Python,firstline = 1, lastline = 45, numbers=left,style = Python]{papers/wwt/code/plot_burglind.py}
	\caption{Python Codeausschnitt}
	\label{fig:python-plot-code}
\end{figure}
\begin{figure}
	\centering
	\lstinputlisting[language=Python,firstline = 46, lastline = 91, numbers=left,style = Python]{papers/wwt/code/plot_burglind.py}
	\caption{Python Codeausschnitt}
