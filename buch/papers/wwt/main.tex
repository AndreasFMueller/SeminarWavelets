...
Setzt man den \textit{Luftdruck} und das Mutter-Wavelet $\psi_{a,b}$ in die Formel \ref{eq:kovarianz_int} ein, kann sie wie folgt umgeformt werden,

\begin{equation}
COV(\text{\textit{Luftdruck}}, \psi_{a,b})(a,b) = \int{(\text{\textit{Luftdruck}}(t)- \overline{\text{\textit{Luftdruck}}})(\psi_{a,b}(t)- \overline{\psi_{a,b}}) \text{\space}dt}.
\end{equation}
...
\begin{equation}
COV(\text{\textit{Luftdruck}}, \psi_{a,b})(a,b) =  \int{\langle \text{\textit{Luftdruck}}(t)- \overline{\text{\textit{Luftdruck}}},\psi_{a,b}(t) \rangle \text{\space}dt}.
\end{equation}
Weiter Ausmultipliziert
\begin{equation}
COV(\text{\textit{Luftdruck}}, \psi_{a,b})(a,b) = \int	\langle \text{\textit{Luftdruck}}(t),\psi_{a,b}(t)\rangle - \langle \overline{\text{\textit{Luftdruck}}},\psi_{a,b}(t)\rangle \text{\space}dt,
\end{equation}
schlussendlich ergibt sich mit $\langle \overline{\text{\textit{Luftdruck}}},\psi_{a,b}(t) \rangle = 0$,
\begin{equation}
\begin{split}
COV(\text{\textit{Luftdruck}}, \psi_{a,b})(a,b) &= \int \langle \text{\textit{Luftdruck}}(t),\psi_{a,b}(t)\rangle \text{\space}dt \\
\end{split}
\end{equation}
...
\end{split}
\end{equation}
ist die Vereinfachung komplett.


\subsection{Multiplikation der Resultate zweier Wavelet-Transformationen}


Die angewendete Wavelet-Transformation mit der Idee des Produktes der beiden Datenkanäle: \newline
...
Ausgeschrieben auf die Vollstädnige Formel der \textit{cwt} (\ref{eq:cwt1})
...
Dabei kommt die Analogie, ohne die Korrekturfaktoren, zu der Formel \ref{eq:cov_wwt} eindrücklich zur Geltung.
...
Um die Formel \ref{eq:produkt-wwt} anzuwenden wurden die beiden gleichgrossen Matrizen des Resultates der \textit{cwt}, des Luftdruckes und des Windes, miteinander elementweise,
...
\begin{figure}[h]
	\centering
	\lstinputlisting[language=Python,firstline = 1, lastline = 18, numbers=left, firstnumber=1, style = Python]{papers/wwt/code/plot_burglind.py}
	\caption{Python Codeausschnitt}
	\label{fig:python-plot-code}
\end{figure}
\begin{figure}[h]
	\centering
	\lstinputlisting[language=Python,firstline = 19, lastline = 70, firstnumber=19, numbers=left,style = Python]{papers/wwt/code/plot_burglind.py}
	\caption{Python Codeausschnitt}
