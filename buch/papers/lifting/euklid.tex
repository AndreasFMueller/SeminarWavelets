...
In jedem Schritt arbeitet man mit zwei Zahlen $a_k$ und $b_k$, die wir
...
Mit dieser Definition lässt sich der euklidische Algorithmus so
beschreiben.

\begin{algorithmus}[Euklid]
\label{lifting:euklid}
Der Algorithmus operiert auf zweidimensionalen Zustandsvektoren $x$ 
wie folgt:
\begin{enumerate}
\item Initialisiere  den Zustandsvektor mit den Zahlen $a$ und $b$:
$\displaystyle x = \begin{pmatrix}a\\b\end{pmatrix}$
\item Bestimme den Quotienten $q$ als die grösste Zahl, für die $qx_2\le x_1$
gilt.
...
Nach Algorithmus~\eqref{lifting:euklid} ist
\[
\begin{pmatrix} g \\ 0 \end{pmatrix}
=
...
$Q$ in der erste Zeile die Zahlen $s$ und $t$ mit denen sich der
