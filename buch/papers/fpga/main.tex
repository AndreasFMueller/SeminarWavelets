...
Figure \ref{fpga:fig:delay} shows the connected blocks. %TODO change this 
...
The results of the Vivado Toolchain and MATLAB match up. 
In figure \ref{fpga:fig:coeff} the coefficients and ready signals before delaying are depicted.
...
Before delaying the ready signal of the fast coefficients was first and the slower ones where delayed. 
...
So after delaying the slow ready signal arrives first and then with every clock cycle the faster ready signals arrive. 
\begin{figure}[h]
	\centering
...
Also interesting is how the coefficients and signals look inside the simulation as shown in figure \ref{fpga:fig:sim}). 
The simulation output makes the recursive multiresolution analysis visible.
%It is clearly visible that the signal losses information in de deeper stages. 
...
% Found an efficient algorithm (lifting scheeme) for a hardware implementation of the DWT

% implemented the Haar wavelet in a forward and inverse multiresolution DWT

%created a test bench to validate our implementation with a equivalent MATLAB simulation
