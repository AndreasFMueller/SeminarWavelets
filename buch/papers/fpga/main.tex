...
While a CPU is bound to its limited instruction set.
A custom hardware can execute multiple operations of choice in one clock cycle.
Some of the operations can be executed in parallel in order to gain even more speed.
...
This decomposition is equivalent to a factorization of the polyphase matrix of the wavelet filter into elementary matrices, as elaborated in the following sections. %TODO Explain in Detail
...
If it is always driven high, the input value is sampled at each clock cycle.
The coefficients are in form of a vector \texttt{d\_s}, which holds $N$ detail coefficients ($b_{j,*} \forall j \in [-1 .. -N]$) and \texttt{s}, which holds the remaining smooth coefficient ($a_{j,*} \forall j = -N$).
Each coeffient is 16bits wide.
...
If the coefficient is updated, the ready signal is driven high.
...
Hereby, the lowpass output (\texttt{s}) of each DFT block (\texttt{haar}) is connected to the input of the next layer DFT block, which performs another transformation to increase detail.

\subsubsection{Delay}

The \texttt{branching} and \texttt{inv\_branching} block are sufficient for an independant forward or inverse transform.
...
\end{figure}
The high frequency coeffients need to be delayed longer, whereas the lower coefficients only need a short delay.
...
%And the wavelet coefficients are also plotted in different colors. %Superfluous?
%The higher frequencies also react faster to changes at the input. %Superfluous?
...
The slower coefficients are calculated first and then the fast ones follow. %TODO uncomprehensible
The slower ones are also longer valid.
The information from the high frequencies are also needed at the moment when the inverse transformation starts. %TODO don't understnad ???? 
...
\end{figure}
As expected, the input and output signals are the same, delayed by the processing time of the system.
The pink line is the result of the MATLAB simulation and the pink dots are the results of the Vivado hardware simulation. %TODO makes more sence if mentioned in first graph of this kind
...
\end{figure}
The simulation shows the delayed coefficients, as well as the ready signals, as they are resembled in the inverse DFT.
The signals are arranged, as they occur in the multiresolution analysis.
As the recursion gets deeper the higher frequencies are subtracted and only the slow frequencies stay. %TODO graph shows inverse transform, not forward transform
...
%TODO too much 'we' .. use passive structure

In our view this project was a success.
We have found an efficient algorithm (lifting scheme) for a hardware implementation of the DWT.
At first, the theory was difficult to understand but finally we have managed to gain insight of the lifting scheme % TODO clumsy sentence
...
Using the testbench, we were able to compare our hardware implementation with a working MATLAB simulation. 
...
All Codes (VHDL, MATLAB) are stored in the github repository belonging to this book. 
\cite{fpga:gitrepo-wavelets}
