...
A in hardware implemented wavelet algorithm has many advantages over a software implementation.
First, arithmetic operations in hardware are much faster than in software.
While a CPU is bound to its limited instruction set.  %TODO: one sentence?
...
\end{align}
If we split these coefficients into even and odd components and fill them into the polyphase matrix we get for the forward transform
...
Multiplications and divisions by a factor of 2 can be efficiently implemented with a bit shift.
...
\end{figure}
The simulation shows the delayed coefficients, as well as the ready signals, as they are reassembled in the inverse DWT.
The signals are arranged, as they occur in the multiresolution analysis.
As the signal is reassembled more and more detail gets added to the signal until the original signal is reconstructed.


\section{Conclusion}
\rhead{Conclusion}

%TODO too much 'we' .. use passive structure  Tried;)
...
At first, the theory was difficult to understand but finally we managed to gain insight in how it works. 
