...
The formula to calculate the forward DTF is %TODO: DWT??
\begin{align}
...
The lifting scheme is a widely used algorithm, which is more efficient in terms of the number of multiplications  used than normal wavelet transformations.
...
This decomposition is equivalent to a factorization of the polyphase matrix of the wavelet filter into elementary matrices. %TODO Explain in Detail
...
The factorization algorithm is of substantial length and is not explained in this work. %TODO: different formulation
...
Hence, the VHDL implementation should feature low latency and high data rate as its main objectives in order to provide a solution that is reasonable as a hardware implementation.
...
So after the delaying the slow ready signal arrives first and then with every clock cycle the faster ready signals arrive. 
\begin{figure}[h]
	\centering
