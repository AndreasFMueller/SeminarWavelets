...
einen Vektor $x\in H$ gibt, der das Minimum realisiert, dass also
$ d = \|y-x\|$.
...
Sei $d=\inf \{ \|y-x\| \;|\;x\in H\}$ der Abstand von $y$ zum Unterraum $V$.
...
lässt sich jeder Vektor $y\in H$ auf genau eine Art in $y = x+u$
zerlegen derart, dass $x \in V$ und $u\in V^{\perp}$,
d.~h.~$\langle z,u\rangle$ für alle $z\in V$.
\end{satz}
...
$k>N$ ist (Siehe auch Abbildung~\ref{satz:orthogonalkomplement}.

Wir zeigen, dass $x_k$ eine Cauchy-Folge ist.
...
Ist
$\|y-x_n\|^2 - d^2  < \varepsilon$ 
und
$\|y-x_m\|^2 - d^2  < \varepsilon$,
...
\text{für $n,m>N.$}
\]
Da man $\varepsilon$ beliebig klein wählen kann, ist $(x_k)_{k\in\mathbb N}$
eine Cauchy-Folge.

Da $(x_k)_{k\in\mathbb N}$ eine Cauchy-Folge ist in $V$ und $V$ ausserdem
abgeschlossen ist, folgt, dass $x_n$ gegen einen Grenzwert 
\[
x=\lim_{k\to\infty} x_k
\]
konvergiert.
$x$ ist der Vektor, für den das Minimum $\|y-u\|=d$ erreicht wird.
Auf diese Weise entsteht eine eindeutige Zerlegung $y = x + u$ mit $u=y-x$.

Es muss jetzt nur noch gezeigt werden, dass $u\in V^{\perp}$.
Dazu wenden wir den Satz~\ref{satz:minimalallgemein} auf die Gerade
durch $x$ mit einem beliebigen Vektor $z\in V$ an.
Da $x$ minimalen Abstand hat, ist $\tilde{x}=x$ unde $z$ muss orthogonal
zu $y-x=u$ sein.
\end{proof}
