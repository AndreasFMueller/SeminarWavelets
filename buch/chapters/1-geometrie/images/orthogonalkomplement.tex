...
\draw[line width=1.4pt] (-1.5,-2)--(6,8);
...
\draw[->,color=blue,line width=1pt] (0,0) -- (Y);

\draw[line width=0.3pt] (3.5+6,0.5+8)--(3.5-6,0.5-8);

\node[color=blue] at ({1.5+2},{2-1.5}) [below] {$y$};

\node at (0,0) [left] {$O$};

\draw[->,line width=1pt,color=red] (Y)--(X);

\fill[color=white] (X) circle[radius=\r];
\draw[line width=1pt,color=red] (X) circle[radius=\r];

\node[color=red] at \v{0.49} [left] {$x$};
